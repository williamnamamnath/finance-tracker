...
export default function Income() {
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <p className="text-green-600 font-semibold mb-6">Total: ${total.toFixed(2)}</p>

      <div className="mb-6">
        <h3 className="text-lg font-semibold mb-2">Income (last 7 days)</h3>
          ...
          <ul className="space-y-2">
            {transactions.map((t) => (
              <li key={t.id} className="p-3 bg-white rounded shadow flex justify-between">
                ...
                <div className="text-green-600 font-semibold">${Number(t.amount).toFixed(2)}</div>
              </li>
            ))}
          </ul>
        )}
